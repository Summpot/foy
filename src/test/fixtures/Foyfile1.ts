...
task('dd', async (ctx) => {
  ...
  logger.debug('p.stdout.trim() === ctx.cwd()', (p.stdout as string).trim() === ctx.cwd)
  logger.debug((p.stdout as string).trim().endsWith('src'))
})

task('ee', async (ctx) => {
  await ctx.exec(['echo aa', 'echo bb'])
})

task('ff', ['aa'.options({ aa: 1 }), 'aa'.options({ aa: 2 })])

const noop = (f) => f
task('notForce', ['aa'.options({ noop }), 'aa'.options({ noop })])
task('force', ['aa'.options({ noop }).force(), 'aa'.options({ noop }).force()])

task<{ t: number }>('wait', async (ctx) => {
  await new Promise((res) => setTimeout(res, ctx.options.t))
  console.log('wait', ctx.options.t)
})
task('sync', ['wait'.options({ t: 100 }), 'wait'.options({ t: 1 })])

task('async', ['wait'.async().options({ t: 100 }), 'wait'.async().options({ t: 1 })])
task('async:priority', ['wait'.async().options({ t: 1 }), 'wait'.async(1).options({ t: 100 })])
