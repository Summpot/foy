...
task("doc", async (ctx) => {
	...
	await ctx.exec(
		`typedoc --theme default --excludePrivate  --out ./docs/api ./src/index.ts`,
	);
...
option("-v, --version <version>", "patch | minor | major", {
	default: "patch",
});
task<{ version: string }>(
	"publish",
	["preversion".options((ctx) => ({ version: ctx.options.version }))],
	async (ctx) => {
		await ctx.exec([
...
task("demo3", ["demo2", "demodemodemodemodemodemodemo1"], async (ctx) =>
	sleep(3000),
);

task("demo", [
	"demodemodemodemodemodemodemo1",
	"demo2".async(),
	"demo3".async(),
]);
